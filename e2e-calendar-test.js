...
(async()=>{
 ...
 try{
  ...
  const occurrence=await recurringEv.getAttribute('data-occurrence'); if(!occurrence||occurrence===today)throw new Error('Future recurrence did not render');
  console.log('RECURRENCE_RENDER_OK');
  const lane=page.locator(`[data-cal-time-lane="${occurrence}"]`); await recurringEv.dragTo(lane,{targetPosition:{x:120,y:56*15}}); await page.waitForTimeout(800);
  ...
  if(!child||!Array.isArray(baseAfter.recurrenceExDates)||!baseAfter.recurrenceExDates.includes(occurrence))throw new Error('Recurring drag did not create a safe exception');
  ...
  await page.mouse.move(box.x+box.width/2,box.y+box.height/2); await page.mouse.down(); await page.mouse.move(box.x+box.width/2,box.y+box.height/2+56,{steps:5}); await page.mouse.up(); await page.waitForTimeout(800);
  ...
  if(toMin(resized.endTime)-toMin(resized.time)<105)throw new Error('Resize did not extend event duration'); console.log('RESIZE_OK');
