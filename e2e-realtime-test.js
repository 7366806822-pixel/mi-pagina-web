...
(async()=>{
 ...
 try{
  ...
  await Promise.all([a.waitForFunction(()=>db.connected===true,{timeout:30000}),b.waitForFunction(()=>db.connected===true,{timeout:30000})]);
  ...
  const fresh=await browser.newContext({viewport:{width:1365,height:900}}),c=await fresh.newPage(); await c.goto(URL,{waitUntil:'domcontentloaded'}); await c.waitForFunction(()=>typeof db!=='undefined'&&db.client&&db.connected===true,{timeout:30000}); await c.waitForFunction(x=>state.records.some(r=>r.id===x&&r.title==='E2E Realtime edición'),id,{timeout:20000}); await fresh.close(); console.log('REOPEN_PERSISTENCE_OK');
  ...
  if(!b.viewportSize()||b.viewportSize().width>500)throw new Error('Mobile viewport not applied'); if(!(await b.title()).includes('Tiempo Real'))throw new Error('Unexpected mobile title'); console.log('CHROME_DESKTOP_MOBILE_OK');
